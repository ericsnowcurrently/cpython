...
PyErr_NewException(const char *name, PyObject *base, PyObject *dict)
{
    ...
    int r = _PyDict_ContainsId(dict, (_Py_Identifier *)&_Py_ID(__module__));
    if (r < 0) {
        goto failure;
    }
    if (r == 0) {
        ...
        if (modulename == NULL)
            goto failure;
        if (_PyDict_SetItemId(dict, (_Py_Identifier *)&_Py_ID(__module__), modulename) != 0)
            goto failure;
    }
                          ...
                          PyObject *err_msg, PyObject *obj, PyObject *file)
{
    ...
    PyObject *modulename = _PyObject_GetAttrId(exc_type, (_Py_Identifier *)&_Py_ID(__module__));
...
_PyErr_WriteUnraisableMsg(const char *err_msg_str, PyObject *obj)
{
    ...
    PyObject *hook = _PySys_GetObjectId((_Py_Identifier *)&_Py_ID(unraisablehook));
    if (hook == NULL) {
        Py_DECREF(hook_args);
        goto default_hook;
    }
                             ...
                             int end_lineno, int end_col_offset)
{
    ...
    if (tmp == NULL)
        ...
        if (_PyObject_SetAttrId(v, (_Py_Identifier *)&_Py_ID(lineno), tmp)) {
            _PyErr_Clear(tstate);
        }
        Py_DECREF(tmp);
    }
    ...
    if (_PyObject_SetAttrId(v, (_Py_Identifier *)&_Py_ID(offset), tmp ? tmp : Py_None)) {
        _PyErr_Clear(tstate);
    }
    Py_XDECREF(tmp);
    ...
    if (_PyObject_SetAttrId(v, (_Py_Identifier *)&_Py_ID(end_lineno), tmp ? tmp : Py_None)) {
        _PyErr_Clear(tstate);
    }
    Py_XDECREF(tmp);
    ...
    if (_PyObject_SetAttrId(v, (_Py_Identifier *)&_Py_ID(end_offset), tmp ? tmp : Py_None)) {
        _PyErr_Clear(tstate);
    }
    Py_XDECREF(tmp);
    ...
    tmp = NULL;
    if (filename != NULL) {
        if (_PyObject_SetAttrId(v, (_Py_Identifier *)&_Py_ID(filename), filename)) {
            _PyErr_Clear(tstate);
        }

        tmp = PyErr_ProgramTextObject(filename, lineno);
        if (tmp) {
            if (_PyObject_SetAttrId(v, (_Py_Identifier *)&_Py_ID(text), tmp)) {
                _PyErr_Clear(tstate);
            }
            Py_DECREF(tmp);
        }
    ...
    if (exc != PyExc_SyntaxError) {
        if (_PyObject_LookupAttrId(v, (_Py_Identifier *)&_Py_ID(msg), &tmp) < 0) {
            _PyErr_Clear(tstate);
        }
        else if (tmp) {
            Py_DECREF(tmp);
            ...
            tmp = PyObject_Str(v);
            if (tmp) {
                if (_PyObject_SetAttrId(v, (_Py_Identifier *)&_Py_ID(msg), tmp)) {
                    _PyErr_Clear(tstate);
                }
                Py_DECREF(tmp);
            }
        ...
        if (_PyObject_LookupAttrId(v, (_Py_Identifier *)&_Py_ID(print_file_and_line), &tmp) < 0) {
            _PyErr_Clear(tstate);
        }
        else if (tmp) {
            Py_DECREF(tmp);
            ...
            if (_PyObject_SetAttrId(v, (_Py_Identifier *)&_Py_ID(print_file_and_line),
                                    Py_None)) {
                _PyErr_Clear(tstate);
            }
        }
