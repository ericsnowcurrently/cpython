...
_Py_FindSourceFile(PyObject *filename, char* namebuf, size_t namelen, PyObject *io)
{
    ...
    syspath = _PySys_GetObjectId((_Py_Identifier *)&_Py_ID(path));
    ...
    for (i = 0; i < npath; i++) {
        ...
        binary = _PyObject_CallMethodId(io, (_Py_Identifier *)&_Py_ID(open), "ss", namebuf, "rb");
        if (binary != NULL) {
            result = binary;
            goto finally;
        }
                                ...
                                int *truncation, PyObject **line)
{
    ...
    if (io == NULL)
        return -1;
    binary = _PyObject_CallMethodId(io, (_Py_Identifier *)&_Py_ID(open), "Os", filename, "rb");
    ...
    fob = _PyObject_CallMethodId(io, (_Py_Identifier *)&_Py_ID(TextIOWrapper), "Os", binary, encoding);
    Py_DECREF(io);
    PyMem_Free(found_encoding);
    ...
    if (fob == NULL) {
        ...
        res = _PyObject_CallMethodIdNoArgs(binary, (_Py_Identifier *)&_Py_ID(close));
        Py_DECREF(binary);
        if (res)
            Py_DECREF(res);
        else
    ...
    res = _PyObject_CallMethodIdNoArgs(fob, (_Py_Identifier *)&_Py_ID(close));
    if (res) {
        Py_DECREF(res);
    }
    else {
