...
_PyRun_InteractiveLoopObject(FILE *fp, PyObject *filename, PyCompilerFlags *flags)
{
    ...
    PyObject *v = _PySys_GetObjectId(&_Py_ID(ps1));
    if (v == NULL) {
        _PySys_SetObjectId(&_Py_ID(ps1), v = PyUnicode_FromString(">>> "));
        Py_XDECREF(v);
    }
    v = _PySys_GetObjectId(&_Py_ID(ps2));
    if (v == NULL) {
        _PySys_SetObjectId(&_Py_ID(ps2), v = PyUnicode_FromString("... "));
        Py_XDECREF(v);
    }
                             ...
                             PyCompilerFlags *flags)
{
    ...
    mod_name = _PyUnicode_FromId(&_Py_ID(__main__)); /* borrowed */
    if (mod_name == NULL) {
        return -1;
    }
    ...
    if (fp == stdin) {
        ...
        v = _PySys_GetObjectId(&_Py_ID(stdin));
        if (v && v != Py_None) {
            oenc = _PyObject_GetAttrId(v, &_Py_ID(encoding));
            if (oenc)
    ...
    v = _PySys_GetObjectId(&_Py_ID(ps1));
    if (v != NULL) {
        v = PyObject_Str(v);
        if (v == NULL)
            PyErr_Clear();
    ...
    w = _PySys_GetObjectId(&_Py_ID(ps2));
    if (w != NULL) {
        w = PyObject_Str(w);
        if (w == NULL)
            PyErr_Clear();
                   ...
                   PyObject **text)
{
    ...
    *message = _PyObject_GetAttrId(err, &_Py_ID(msg));
    if (!*message)
        goto finally;

    v = _PyObject_GetAttrId(err, &_Py_ID(filename));
    if (!v)
        goto finally;
    if (v == Py_None) {
        Py_DECREF(v);
        *filename = _PyUnicode_FromId(&_Py_ID(string));
        if (*filename == NULL)
            goto finally;
        Py_INCREF(*filename);
    }
    ...
    v = _PyObject_GetAttrId(err, &_Py_ID(lineno));
    if (!v)
        goto finally;
    ...
    *lineno = hold;
    ...
    v = _PyObject_GetAttrId(err, &_Py_ID(offset));
    if (!v)
        goto finally;
    if (v == Py_None) {
        *offset = -1;
    ...
    if (Py_TYPE(err) == (PyTypeObject*)PyExc_SyntaxError) {
        v = _PyObject_GetAttrId(err, &_Py_ID(end_lineno));
        if (!v) {
            PyErr_Clear();
            *end_lineno = *lineno;
        }
        ...
        v = _PyObject_GetAttrId(err, &_Py_ID(end_offset));
        if (!v) {
            PyErr_Clear();
            *end_offset = -1;
        }
    ...
    v = _PyObject_GetAttrId(err, &_Py_ID(text));
    if (!v)
        goto finally;
    if (v == Py_None) {
        Py_DECREF(v);
...
_Py_HandleSystemExit(int *exitcode_p)
{
    ...
    if (PyExceptionInstance_Check(value)) {
        ...
        _Py_IDENTIFIER(code);
        PyObject *code = _PyObject_GetAttrId(value, &_Py_ID(code));
        if (code) {
            Py_DECREF(value);
            value = code;
            if (value == Py_None)
    ...
    if (PyLong_Check(value)) {
        ...
        PyObject *sys_stderr = _PySys_GetObjectId(&_Py_ID(stderr));
...
_PyErr_PrintEx(PyThreadState *tstate, int set_sys_last_vars)
{
    ...
    if (set_sys_last_vars) {
        if (_PySys_SetObjectId(&_Py_ID(last_type), exception) < 0) {
            _PyErr_Clear(tstate);
        }
        if (_PySys_SetObjectId(&_Py_ID(last_value), v) < 0) {
            _PyErr_Clear(tstate);
        }
        if (_PySys_SetObjectId(&_Py_ID(last_traceback), tb) < 0) {
            _PyErr_Clear(tstate);
        }
    }
    hook = _PySys_GetObjectId(&_Py_ID(excepthook));
                              ...
                              PyObject **value_p)
{
    ...
    PyObject *tmp;
    int res = _PyObject_LookupAttrId(*value_p, &_Py_ID(print_file_and_line), &tmp);
    if (res <= 0) {
        if (res < 0) {
            PyErr_Clear();
        }
                        ...
                        PyObject *value)
{
    ...
    PyObject *modulename = _PyObject_GetAttrId(type, &_Py_ID(__module__));
...
print_exception_note(struct exception_print_context *ctx, PyObject *value)
{
    ...
    PyObject *note = _PyObject_GetAttrId(value, &_Py_ID(__note__));
    if (note == NULL) {
        return -1;
    }
                  ...
                  PyObject *locals, PyCompilerFlags *flags)
{
    ...
    filename = _PyUnicode_FromId(&_Py_ID(string)); /* borrowed */
    if (filename == NULL)
        return NULL;
...
flush_io(void)
{
    ...
    f = _PySys_GetObjectId(&_Py_ID(stderr));
    if (f != NULL) {
        r = _PyObject_CallMethodIdNoArgs(f, &_Py_ID(flush));
        if (r)
            Py_DECREF(r);
        else
            PyErr_Clear();
    }
    f = _PySys_GetObjectId(&_Py_ID(stdout));
    if (f != NULL) {
        r = _PyObject_CallMethodIdNoArgs(f, &_Py_ID(flush));
        if (r)
            Py_DECREF(r);
