                 ...
                 const char *argFormat, va_list vargs)
{
    ...
    if (is->audit_hooks) {
        ...
        while ((hook = PyIter_Next(hooks)) != NULL) {
            ...
            PyObject *o;
            int canTrace = _PyObject_LookupAttrId(hook, (_Py_Identifier *)&_Py_ID(__cantrace__), &o);
            if (o) {
...
sys_displayhook_unencodable(PyObject *outf, PyObject *o)
{
    ...
    stdout_encoding = _PyObject_GetAttrId(outf, (_Py_Identifier *)&_Py_ID(encoding));
    ...
    if (_PyObject_LookupAttrId(outf, (_Py_Identifier *)&_Py_ID(buffer), &buffer) < 0) {
        Py_DECREF(encoded);
        goto error;
    }
    if (buffer) {
        result = _PyObject_CallMethodIdOneArg(buffer, (_Py_Identifier *)&_Py_ID(write), encoded);
        Py_DECREF(buffer);
        Py_DECREF(encoded);
        if (result == NULL)
            goto error;
...
/*[clinic end generated code: output=347477d006df92ed input=08ba730166d7ef72]*/
{
    ...
    builtins = _PyImport_GetModuleId((_Py_Identifier *)&_Py_ID(builtins));
    if (builtins == NULL) {
    ...
    if (_PyObject_SetAttrId(builtins, (_Py_Identifier *)&_Py_ID(_), Py_None) != 0)
        return NULL;
    outf = sys_get_object_id(tstate, (_Py_Identifier *)&_Py_ID(stdout));
    ...
    if (PyFile_WriteObject(newline, outf, Py_PRINT_RAW) != 0)
        return NULL;
    if (_PyObject_SetAttrId(builtins, (_Py_Identifier *)&_Py_ID(_), o) != 0)
        return NULL;
    Py_RETURN_NONE;
}
...
_PySys_GetSizeOf(PyObject *o)
{
    ...
    method = _PyObject_LookupSpecial(o, (_Py_Identifier *)&_Py_ID(__sizeof__));
    if (method == NULL) {
        if (!_PyErr_Occurred(tstate)) {
            _PyErr_Format(tstate, PyExc_TypeError,
...
{
    PyObject *warnoptions = sys_get_object_id(tstate, (_Py_Identifier *)&_Py_ID(warnoptions));
    if (warnoptions == NULL || !PyList_Check(warnoptions)) {
        ...
        *  interpreter config. When that happens, we need to properly set
        ...
        if (sys_set_object_id(tstate->interp, (_Py_Identifier *)&_Py_ID(warnoptions), warnoptions)) {
...
{
    PyObject *xoptions = sys_get_object_id(tstate, (_Py_Identifier *)&_Py_ID(_xoptions));
    if (xoptions == NULL || !PyDict_Check(xoptions)) {
        ...
        *  interpreter config. When that happens, we need to properly set
        ...
        if (sys_set_object_id(tstate->interp, (_Py_Identifier *)&_Py_ID(_xoptions), xoptions)) {
            Py_DECREF(xoptions);
            return NULL;
        }
        Py_DECREF(xoptions);
...
_PySys_SetPreliminaryStderr(PyObject *sysdict)
{
    ...
    if (_PyDict_SetItemId(sysdict, (_Py_Identifier *)&_Py_ID(stderr), pstderr) < 0) {
        goto error;
    }
...
PySys_SetPath(const wchar_t *path)
{
    ...
    PyInterpreterState *interp = _PyInterpreterState_GET();
    if (sys_set_object_id(interp, (_Py_Identifier *)&_Py_ID(path), v) != 0) {
...
PySys_SetArgvEx(int argc, wchar_t **argv, int updatepath)
{
    ...
    if (updatepath) {
        ...
        if (_PyPathConfig_ComputeSysPath0(&argv_list, &path0)) {
            ...
            PyObject *sys_path = sys_get_object_id(tstate, (_Py_Identifier *)&_Py_ID(path));
            if (sys_path != NULL) {
...
sys_pyfile_write_unicode(PyObject *unicode, PyObject *file)
{
    if (file == NULL)
        return -1;
    assert(unicode != NULL);
    PyObject *result = _PyObject_CallMethodIdOneArg(file, (_Py_Identifier *)&_Py_ID(write), unicode);
    if (result == NULL) {
        return -1;
    }
    Py_DECREF(result);
...
PySys_WriteStdout(const char *format, ...)
{
    ...
    va_start(va, format);
    sys_write((_Py_Identifier *)&_Py_ID(stdout), stdout, format, va);
    va_end(va);
}
...
PySys_WriteStderr(const char *format, ...)
{
    ...
    va_start(va, format);
    sys_write((_Py_Identifier *)&_Py_ID(stderr), stderr, format, va);
    va_end(va);
}
...
PySys_FormatStdout(const char *format, ...)
{
    ...
    va_start(va, format);
    sys_format((_Py_Identifier *)&_Py_ID(stdout), stdout, format, va);
    va_end(va);
}
...
PySys_FormatStderr(const char *format, ...)
{
    ...
    va_start(va, format);
    sys_format((_Py_Identifier *)&_Py_ID(stderr), stderr, format, va);
    va_end(va);
}
