                           ...
                           _PyXIData_t *data)
{
    ...
    int res = _PyObject_GetXIData(tstate, wrapped, fallback, data);
    Py_DECREF(wrapped);
    if (res < 0) {
        goto error;
    }
...
error:
    ...
    PyObject *cause = _PyErr_GetRaisedException(tstate);
    assert(cause != NULL);
    set_notshareableerror(tstate, cause, 0, "error while wrapping object");
    Py_DECREF(cause);
    return -1;
}
...
_xidwrapper_handle_used(PyThreadState *tstate, xidwrapper *wrapper)
{
    ...
    set_notshareableerror(tstate, NULL, 0, "wrapped data already used");
    return -1;
}
                  ...
                  _PyXIData_t *data)
{
    ...
    if (wrapper->wrapfunc != NULL || wrapper->fallback) {
        ...
        xidata_fallback_t fallback = _PyXIDATA_XIDATA_ONLY;
        return _PyObject_GetXIData(tstate, obj, fallback, data);
    }
}
...
cross-interpreter data ahead of time.  CrossIntpreterObjectData\n\
...
_xidwrapper_register_type(PyInterpreterState *interp)
{
    ...
    _PyXIData_getdata_t getdata = {.basic=(xidwrapper_shared)};
    if (_PyXIData_RegisterClass(tstate, &_PyXIDataWrapper_Type, getdata) < 0) {
        _PyStaticType_FiniBuiltin(interp, &_PyXIDataWrapper_Type);
        return -1;
    }
    return 0;
