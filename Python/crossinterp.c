...
_handle_unpickle_missing_attr(struct _unpickle_context *ctx, PyObject *exc)
{
    ...
    // We should have been able to reach here only if it wasn't set,
    // so we bail out if it is unexpectedly set already.
    if (PyObject_HasAttrString(mod.module, info.attrname)) {
        assert(_PyErr_Occurred(ctx->tstate));
        if (!_PyErr_Occurred(ctx->tstate)) {
            PyErr_Format(PyExc_RuntimeError,
                         "unexpected attr %s", info.attrname);
        }
        return -1;
    }
