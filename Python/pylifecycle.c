...
finalize_clear_modules_dict(PyObject *modules)
{
    if (PyDict_CheckExact(modules)) {
        ...
        if (_PyObject_CallMethodIdNoArgs(modules, (_Py_Identifier *)&_Py_ID(clear)) == NULL) {
            PyErr_WriteUnraisable(NULL);
        }
    }
}
    ...
    const wchar_t* encoding, const wchar_t* errors)
{
    ...
    */
    ...
    if (write_mode)
        ...
        mode = "rb";
    buf = _PyObject_CallMethodId(io, (_Py_Identifier *)&_Py_ID(open), "isiOOOO",
                                 fd, mode, buffering,
    ...
    if (buffering) {
        _Py_IDENTIFIER(raw);
        raw = _PyObject_GetAttrId(buf, (_Py_Identifier *)&_Py_ID(raw));
        if (raw == NULL)
            goto error;
    }
    else {
    ...
    if (text == NULL || _PyObject_SetAttrId(raw, (_Py_Identifier *)&_Py_ID(name), text) < 0)
        goto error;
    res = _PyObject_CallMethodIdNoArgs(raw, (_Py_Identifier *)&_Py_ID(isatty));
    if (res == NULL)
        goto error;
    ...
    stream = _PyObject_CallMethodId(io, (_Py_Identifier *)&_Py_ID(TextIOWrapper), "OOOsOO",
    ...
    if (!text || _PyObject_SetAttrId(stream, (_Py_Identifier *)&_Py_ID(mode), text) < 0)
        goto error;
    Py_CLEAR(text);
    return stream;
...
init_sys_streams(PyThreadState *tstate)
{
    ...
    _PySys_SetObjectId((_Py_Identifier *)&_Py_ID(stdin), std);
    Py_DECREF(std);
    ...
    _PySys_SetObjectId((_Py_Identifier *)&_Py_ID(stdout), std);
    Py_DECREF(std);
    ...
    if (_PySys_SetObjectId((_Py_Identifier *)&_Py_ID(stderr), std) < 0) {
        Py_DECREF(std);
        goto error;
    }
    Py_DECREF(std);
...
_Py_FatalError_PrintExc(PyThreadState *tstate)
{
    ...
    ferr = _PySys_GetObjectId((_Py_Identifier *)&_Py_ID(stderr));
    ...
    res = _PyObject_CallMethodIdNoArgs(ferr, (_Py_Identifier *)&_Py_ID(flush));
    if (res == NULL) {
        _PyErr_Clear(tstate);
    }
    else {
...
wait_for_thread_shutdown(PyThreadState *tstate)
{
    ...
    PyObject *result;
    PyObject *threading = _PyImport_GetModuleId((_Py_Identifier *)&_Py_ID(threading));
    if (threading == NULL) {
        if (_PyErr_Occurred(tstate)) {
            PyErr_WriteUnraisable(NULL);
        }
    ...
    result = _PyObject_CallMethodIdNoArgs(threading, (_Py_Identifier *)&_Py_ID(_shutdown));
    if (result == NULL) {
