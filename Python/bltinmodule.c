...
update_bases(PyObject *bases, PyObject *const *args, Py_ssize_t nargs)
{
    ...
    for (i = 0; i < nargs; i++) {
        ...
        if (_PyObject_LookupAttrId(base, &_Py_ID(__mro_entries__), &meth) < 0) {
            goto error;
        }
        if (!meth) {
            if (new_bases) {
                        ...
                        PyObject *kwnames)
{
    ...
    if (kwnames == NULL) {
        ...
        meta = _PyDict_GetItemIdWithError(mkw, &_Py_ID(metaclass));
        if (meta != NULL) {
            Py_INCREF(meta);
            if (_PyDict_DelItemId(mkw, &_Py_ID(metaclass)) < 0) {
                goto error;
            }
    ...
    if (_PyObject_LookupAttrId(meta, &_Py_ID(__prepare__), &prep) < 0) {
        ns = NULL;
    }
    else if (prep == NULL) {
        ns = PyDict_New();
...
/*[clinic end generated code: output=0a0824aa70093116 input=11ee718a8640e527]*/
{
    ...
    int r = _PyDict_ContainsId(globals, &_Py_ID(__builtins__));
    if (r == 0) {
        r = _PyDict_SetItemId(globals, &_Py_ID(__builtins__),
                              PyEval_GetBuiltins());
    }
    if (r < 0) {
        return NULL;
...
/*[clinic end generated code: output=3cfc0940f5bc237b input=c143c575d24fe665]*/
{
    ...
    if (file == Py_None) {
        file = _PySys_GetObjectId(&_Py_ID(stdout));
        if (file == NULL) {
    ...
    if (flush) {
        PyObject *tmp = _PyObject_CallMethodIdNoArgs(file, &_Py_ID(flush));
        if (tmp == NULL) {
            return NULL;
        }
        Py_DECREF(tmp);
...
{
    PyObject *fin = _PySys_GetObjectId(&_Py_ID(stdin));
    PyObject *fout = _PySys_GetObjectId(&_Py_ID(stdout));
    PyObject *ferr = _PySys_GetObjectId(&_Py_ID(stderr));
    PyObject *tmp;
    long fd;
    int tty;
    ...
    tmp = _PyObject_CallMethodIdNoArgs(ferr, &_Py_ID(flush));
    if (tmp == NULL)
        PyErr_Clear();
    else
        Py_DECREF(tmp);
    ...
    tmp = _PyObject_CallMethodIdNoArgs(fin, &_Py_ID(fileno));
    if (tmp == NULL) {
        PyErr_Clear();
        tty = 0;
    }
    ...
    if (tty) {
        tmp = _PyObject_CallMethodIdNoArgs(fout, &_Py_ID(fileno));
        if (tmp == NULL) {
            PyErr_Clear();
            tty = 0;
        }
    ...
    if (tty) {
        ...
        stdin_encoding = _PyObject_GetAttrId(fin, &_Py_ID(encoding));
        stdin_errors = _PyObject_GetAttrId(fin, &_Py_ID(errors));
        ...
        if (!stdin_encoding_str || !stdin_errors_str)
            goto _readline_errors;
        tmp = _PyObject_CallMethodIdNoArgs(fout, &_Py_ID(flush));
        if (tmp == NULL)
            PyErr_Clear();
        else
            Py_DECREF(tmp);
        if (prompt != NULL) {
            ...
            PyObject *stringpo;
            stdout_encoding = _PyObject_GetAttrId(fout, &_Py_ID(encoding));
            stdout_errors = _PyObject_GetAttrId(fout, &_Py_ID(errors));
...
builtin_sorted(PyObject *self, PyObject *const *args, Py_ssize_t nargs, PyObject *kwnames)
{
    ...
    callable = _PyObject_GetAttrId(newlist, &_Py_ID(sort));
    if (callable == NULL) {
        Py_DECREF(newlist);
        return NULL;
    }
...
builtin_vars(PyObject *self, PyObject *args)
{
    ...
    if (v == NULL) {
        ...
        if (_PyObject_LookupAttrId(v, &_Py_ID(__dict__), &d) == 0) {
            PyErr_SetString(PyExc_TypeError,
