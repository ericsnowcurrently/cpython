...
StructUnionType_new(PyTypeObject *type, PyObject *args, PyObject *kwds, int isStruct)
{
    ...
    int r = _PyDict_ContainsId(result->tp_dict, (_Py_Identifier *)&_Py_ID(_abstract_));
    if (r > 0)
    ...
    fields = _PyDict_GetItemIdWithError((PyObject *)dict, (_Py_Identifier *)&_Py_ID(_fields_));
    if (fields) {
        if (_PyObject_SetAttrId((PyObject *)result, (_Py_Identifier *)&_Py_ID(_fields_), fields) < 0) {
            Py_DECREF(result);
            return NULL;
        }
        return (PyObject *)result;
...
CDataType_from_param(PyObject *type, PyObject *value)
{
    ...
    if (_PyObject_LookupAttrId(value, (_Py_Identifier *)&_Py_ID(_as_parameter_), &as_parameter) < 0) {
        return NULL;
    }
    if (as_parameter) {
...
*/
    ...
    proto = _PyDict_GetItemIdWithError(typedict, (_Py_Identifier *)&_Py_ID(_type_)); /* Borrowed ref */
    if (proto) {
...
PyCPointerType_set_type(PyTypeObject *self, PyObject *type)
{
    ...
    if (-1 == _PyDict_SetItemId((PyObject *)dict, (_Py_Identifier *)&_Py_ID(_type_), type))
        return NULL;
...
PyCArrayType_new(PyTypeObject *type, PyObject *args, PyObject *kwds)
{
    ...
    if (_PyObject_LookupAttrId((PyObject *)result, (_Py_Identifier *)&_Py_ID(_length_), &length_attr) < 0) {
        goto error;
    }
    if (!length_attr) {
    ...
    if (_PyObject_LookupAttrId((PyObject *)result, (_Py_Identifier *)&_Py_ID(_type_), &type_attr) < 0) {
        goto error;
    }
    if (!type_attr) {
        PyErr_SetString(PyExc_AttributeError,
...
PyCSimpleType_new(PyTypeObject *type, PyObject *args, PyObject *kwds)
{
    ...
    if (_PyObject_LookupAttrId((PyObject *)result, (_Py_Identifier *)&_Py_ID(_type_), &proto) < 0) {
        return NULL;
    }
    if (!proto) {
        PyErr_SetString(PyExc_AttributeError,
 ...
 */
        ...
        if (_PyObject_LookupAttrId(tp, (_Py_Identifier *)&_Py_ID(from_param), &cnv) <= 0) {
            Py_DECREF(converters);
            Py_DECREF(ob);
            if (!PyErr_Occurred()) {
                PyErr_Format(PyExc_TypeError,
...
make_funcptrtype_dict(StgDictObject *stgdict)
{
    ...
    ob = _PyDict_GetItemIdWithError((PyObject *)stgdict, (_Py_Identifier *)&_Py_ID(_flags_));
    ...
    ob = _PyDict_GetItemIdWithError((PyObject *)stgdict, (_Py_Identifier *)&_Py_ID(_argtypes_));
    if (ob) {
    ...
    ob = _PyDict_GetItemIdWithError((PyObject *)stgdict, (_Py_Identifier *)&_Py_ID(_restype_));
    if (ob) {
        ...
        Py_INCREF(ob);
        stgdict->restype = ob;
        if (_PyObject_LookupAttrId(ob, (_Py_Identifier *)&_Py_ID(_check_retval_),
                                   &stgdict->checker) < 0)
        {
            return -1;
        }
...
/* XXX later, maybe.
    ob = _PyDict_GetItemIdWithError((PyObject *)stgdict, (_Py_Identifier *)&_Py_ID(_errcheck_));
    if (ob) {
...
PyCFuncPtr_set_restype(PyCFuncPtrObject *self, PyObject *ob, void *Py_UNUSED(ignored))
{
    ...
    if (_PyObject_LookupAttrId(ob, (_Py_Identifier *)&_Py_ID(_check_retval_), &checker) < 0) {
        return -1;
    }
    oldchecker = self->checker;
...
PyCFuncPtr_new(PyTypeObject *type, PyObject *args, PyObject *kwds)
{
    ...
    */
/*
    if (kwds && _PyDict_GetItemIdWithError(kwds, (_Py_Identifier *)&_Py_ID(options))) {
        ...
    }
    else if (PyErr_Occurred()) {
        return NULL;
              ...
              int outmask, int inoutmask, unsigned int numretvals)
{
    ...
    for (bit = 1, i = 0; i < 32; ++i, bit <<= 1) {
        ...
        if (bit & inoutmask) {
            ...
            v = PyTuple_GET_ITEM(callargs, i);
            v = _PyObject_CallMethodIdNoArgs(v, (_Py_Identifier *)&_Py_ID(__ctypes_from_outparam__));
