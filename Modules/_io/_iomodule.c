...
/*[clinic end generated code: output=aefafc4ce2b46dc0 input=1543f4511d2356a5]*/
{
    ...
    *m = '\0';
    ...
    /* buffering */
    if (buffering < 0) {
        PyObject *res = _PyObject_CallMethodIdNoArgs(raw, (_Py_Identifier *)&_Py_ID(isatty));
        if (res == NULL)
            goto error;
    ...
    if (buffering < 0) {
        ...
        blksize_obj = _PyObject_GetAttrId(raw, (_Py_Identifier *)&_Py_ID(_blksize));
        if (blksize_obj == NULL)
            goto error;
    ...
    if (_PyObject_SetAttrId(wrapper, (_Py_Identifier *)&_Py_ID(mode), modeobj) < 0)
        goto error;
    Py_DECREF(modeobj);
    return result;
  ...
  error:
    if (result != NULL) {
        ...
        close_result = _PyObject_CallMethodIdNoArgs(result, (_Py_Identifier *)&_Py_ID(close));
