...
faulthandler_get_fileno(PyObject **file_ptr)
{
    ...
    if (file == NULL || file == Py_None) {
        file = _PySys_GetObjectId((_Py_Identifier *)&_Py_ID(stderr));
        if (file == NULL) {
    ...
    result = _PyObject_CallMethodIdNoArgs(file, (_Py_Identifier *)&_Py_ID(fileno));
    if (result == NULL)
        return -1;
    ...
    result = _PyObject_CallMethodIdNoArgs(file, (_Py_Identifier *)&_Py_ID(flush));
    if (result != NULL)
        Py_DECREF(result);
    else {
        /* ignore flush() error */
...
faulthandler_init_enable(void)
{
    ...
    PyObject *res = _PyObject_CallMethodIdNoArgs(module, (_Py_Identifier *)&_Py_ID(enable));
    Py_DECREF(module);
    if (res == NULL) {
        return -1;
    }
