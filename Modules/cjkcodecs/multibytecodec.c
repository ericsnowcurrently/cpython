                      ...
                      PyObject *unistr)
{
    ...
    wr = _PyObject_CallMethodIdOneArg(self->stream, (_Py_Identifier *)&_Py_ID(write), str);
    Py_DECREF(str);
    if (wr == NULL)
        return -1;
...
/*[clinic end generated code: output=8f54a4d9b03db5ff input=b56dbcbaf35cc10c]*/
{
    ...
    if (PyBytes_Size(pwrt) > 0) {
        ...
        wr = _PyObject_CallMethodIdOneArg(self->stream, (_Py_Identifier *)&_Py_ID(write), pwrt);
        if (wr == NULL) {
            Py_DECREF(pwrt);
            return NULL;
        }
