...
static FNFCISTATUS(cb_status)
{
    if (pv) {
        ...
        PyObject *result = _PyObject_CallMethodId(pv, (_Py_Identifier *)&_Py_ID(status), "iii", typeStatus, cb1, cb2);
        if (result == NULL)
            return -1;
        Py_DECREF(result);
    }
...
static FNFCIGETNEXTCABINET(cb_getnextcabinet)
{
    if (pv) {
        ...
        PyObject *result = _PyObject_CallMethodId(pv, (_Py_Identifier *)&_Py_ID(getnextcabinet), "i", pccab->iCab);
        if (result == NULL)
            return -1;
